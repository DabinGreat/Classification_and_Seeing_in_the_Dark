...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        return {'video_x': torch.from_numpy(video_x), 'video_label': torch.LongTensor(video_label)}
...
if __name__ == '__main__':
    ...
    for i_batch, sample_batched in enumerate(dataloader):
        if i_batch==1:
            print(i_batch,
                  sample_batched['video_x'].size(),
                  sample_batched['video_x'].type(),
                  sample_batched['video_x'],
                  sample_batched['video_label'].size())
